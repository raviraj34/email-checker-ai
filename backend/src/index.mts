...
async function checkDomain(domain: string): Promise<boolean> {
    ...
    try {
        ...
        const data = await respose.json() as { Answer: any[] };
...
async function analyzeDomainWithAI(domain: string): Promise<AIAnalysisResult> {
    ...
    try {
        ...
        const parsedJson: AIAnalysisResult = JSON.parse(rawText);
